...
- **범위 기반 변동성 추정량** (P16) — 고가·저가를 쓰면 같은 데이터에서 훨씬 정확합니다
  (직접 측정한 효율: Parkinson 5.2배, Garman–Klass 8.2배, Rogers–Satchell 6.4배).
  기본값은 Rogers–Satchell과 Yang–Zhang입니다 — Parkinson·Garman–Klass는 드리프트를
  0으로 가정하고, 실제로 드리프트를 올리면 Parkinson의 편의가 +30%까지 벌어집니다.
  드리프트와 분산의 상호작용을 다루는 도구가 드리프트 0을 가정한 추정량을 쓸 수는 없습니다.
  다만 범위 기반 추정량은 **모두 하방 편의**를 갖고 그 크기가 **유동성과 상관**되므로
  (체결이 성길수록 관측 범위가 좁아짐), 도입 시 횡단면 순위가 유동성 방향으로 왜곡될
  위험이 있습니다. 종가 기반 값과 나란히 보고할 계획입니다.
- **핵심 주장의 측정** (P17) — 이 README는 "σ²이 μ보다 정확하다"고 반복해 왔지만
  한 번도 측정한 적이 없습니다. 창 t의 추정치가 창 t+1의 실현값을 얼마나 예측하는지
  추정량별로 재서, 주장을 숫자로 바꿉니다. 결과가 불리하게 나와도 그대로 싣습니다.
- **다중검정 보정** (P18) — 1,106종목 × 3검정을 5%로 돌리면 우연히 55건쯤이 «유의»해집니다.
  Benjamini–Hochberg FDR로 원시 기각 수와 보정 후 기각 수를 함께 보고합니다.
